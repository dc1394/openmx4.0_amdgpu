...
dcomplex *Hs11_Cx,*Hs22_Cx,*Hs12_Cx;
...
{
    /* The packed H/S cache feeds the dense-GPU band paths AND the collinear
       cluster solver (Cluster_DFT_Col consumes it whenever it is ready; a
       gamma-point-only band input is rerouted to Solver==2 by Input_std, so
       it lands there too).  Only the BAND consumers are demoted on an
       oversubscribed GPU (their switch number is forced unreachable), so only
       Solver==3 may skip the build then - that also avoids the large
       MPI_Gatherv that trips UCX's shared-memory assertion in that regime.
       For Solver==2 the cache must always be built: without it the cluster
       path takes a rarely exercised fallback that produced wrong total
       energies for large systems (5_5_13COb2, GFRAG in -runtestL) at
       24 ranks per GPU. */
    if (scf_eigen_lib_flag == GPUSOLVER &&
        (Solver == 2 ||
         (Solver == 3 && !openmx_band_gpu_dense_oversubscribed()))) {
...
double DFT(int MD_iter, int Cnt_Now)
{
  ...
  *************************************************************/
  ...
  if      (Solver==2 && SpinP_switch<=1)  Allocate_Free_Cluster_Col(1);        
  ...
  *******************************************************************/
  ...
  if (Solver==2 && SpinP_switch==3){
    ...
    time5 += Cluster_DFT_NonCol_ScatterGpuSolverCachedEVec(n*2,is2,ie2,EVec1_NonCol);
  ...
  /*******************************************************************
      calculation of the electric polarization by the analytic 
      Berry connection formula for the occupied space
  *******************************************************************/

  if (Polarization_ABC_flag==1){

    if ( (Solver==2 || Solver==3) && SpinP_switch<=1){

      time5 += Electric_Polarization_BandCol(LSCF_iter,
					     Kspace_grid1,Kspace_grid2,Kspace_grid3,
					     SpinP_switch,H,iHNL,OLP[0],DM[0],EDM,Eele0,Eele1, 
					     MP,order_GA,ko,koS,EIGEN_Band,
					     H1,S1,
    ...
    else if ( (Solver==2 || Solver==3) && SpinP_switch==3){

      time5 += Electric_Polarization_BandNonCol(LSCF_iter,
						Kspace_grid1,Kspace_grid2,Kspace_grid3,
						SpinP_switch,H,iHNL,OLP[0],DM[0],EDM,Eele0,Eele1, 
						MP,order_GA,ko_noncol,koS,EIGEN_Band,
						H1,
  ...
  *******************************************************************/
  ...
  if (CWF_Calc==1){
    ...
    if ( Solver==2 && SpinP_switch<=1 ){
      ...
      /* For CWF and MO guiding functions */

      if (CWF_Calc==1 && CWF_Guiding_Orbital==3){

        if (MYID_MPI_COMM_WORLD==Host_ID  && 0<level_stdout){
          printf("\n<DFT>  Calculation of MO-type guiding functions for CWFs\n");fflush(stdout);
	}

	time5 += Cluster_DFT_Col("ParDM",2,SpinP_switch,
				 ko_col,H,OLP[0],DM[0],EDM,
				 Eele0,Eele1,
				 myworld1,NPROCS_ID1,Comm_World1,NPROCS_WD1,
				 Comm_World_StartID1,MPI_CommWD1,MP,is2,ie2,
	...
	time5 += Cluster_DFT_NonCol("ParDM",LSCF_iter,SpinP_switch,
				    ko_noncol,H,iHNL,OLP[0],DM[0],EDM,
				    Eele0,Eele1,
				    MP,is2,ie2,
				    Ss_Re,Cs_Re,
    ...
    /* generating interface files to Wannier90, 
       such as mmn, ammn, eig, win */

    if (cwf2w90_output_flag){
      CWF2W90(k_op,T_k_op,T_k_ID,T_KGrids1,T_KGrids2,T_KGrids3,MP,order_GA);
    }
  ...
  *****************************************************/
  ...
  if (Scf_RestartToFile && SpinP_switch!=3){
  ...
  ****************************************************/
  ...
  if (!orbitalOpt_Force_Skip) time8 += Total_Energy(MD_iter,ECE);
  ...
  /*****************************************************
                output files for flpq
   The correction:  
   DFT.c
   iDM is added in calling TRAN_DFT()
   TRAN_DFT.c
   tran_prototypes.h
   flpq_dm.h
   out_openmx_lpq.c
        ...
        switch (Solver) {
          ...
          case 2:
            ...
            if (SpinP_switch<=1){
              ...
              Cluster_DFT_Col_DMmu("scf",LSCF_iter,SpinP_switch,
                           ko_col,H,OLP[0],DMmu,
                           myworld1,NPROCS_ID1,Comm_World1,NPROCS_WD1,
                           Comm_World_StartID1,MPI_CommWD1,MP,is2,ie2,
                           Ss_Re,Cs_Re,Hs_Re,
              ...
              Cluster_DFT_NonCol_DMmu("scf",LSCF_iter,SpinP_switch,
                              ko_noncol,H,iHNL,OLP[0],DMmu,iDMmu,
                              MP,is2,ie2,
                              Ss_Re,Cs_Re,
                              rHs11_Re,rHs12_Re,rHs22_Re,iHs11_Re,iHs12_Re,iHs22_Re,
                              Ss2_Cx,Hs2_Cx,Cs2_Cx,
	      ...
	      Band_DFT_NonCol_DMmu(LSCF_iter,
				   Kspace_grid1,Kspace_grid2,Kspace_grid3,
				   SpinP_switch,H,iHNL,OLP[0],DMmu,iDMmu, 
				   MP,order_GA,ko_noncol,koS,EIGEN_Band,
				   H1,
				   Hs11_Cx,Hs22_Cx,Hs12_Cx,
				   EVec1_Cx,
        ...
        switch (Solver) {
          ...
          case 3:
            if (SpinP_switch<=1){
              Band_DFT_Col_DMmu(LSCF_iter,
                              Kspace_grid1,Kspace_grid2,Kspace_grid3,
                              SpinP_switch,CntH,iCntHNL,CntOLP[0],DMmu, 
                              MP,order_GA,ko,koS,EIGEN_Band,
                              H1,S1,
                              CDM1,
                              EVec1_Cx,
	      ...
	      Band_DFT_NonCol_DMmu(LSCF_iter,
				   Kspace_grid1,Kspace_grid2,Kspace_grid3,
				   SpinP_switch,CntH,iCntHNL,CntOLP[0],DMmu,iDMmu, 
				   MP,order_GA,ko_noncol,koS,EIGEN_Band,
				   H1,
				   Hs11_Cx,Hs22_Cx,Hs12_Cx,
				   EVec1_Cx,
  ...
  *******************************************************************/
  ...
  if      (Solver==2 && SpinP_switch<=1)  Allocate_Free_Cluster_Col(2);        
    ...
    Divide_Conquer_LNO( "free",MD_iter,LSCF_iter,SucceedReadingDMfile,
		         H,iHNL,OLP[0],DM[0],EDM,Eele0,Eele1 );
  }
  ...
  *********************************************************/
  ...
  if ( (xanes_gs_fileout==1 || xanes_calc==1) && SpinP_switch<=1 ){
    ...
    if      (xanes_gs_fileout==1) sprintf(command,"lcaoout");
    else if (xanes_calc==1)       sprintf(command,"xanes");
    ...
    time5 += Cluster_DFT_Col(command,1,SpinP_switch,
			     ko_col,H,OLP[0],DM[0],EDM,
			     Eele0,Eele1,
			     myworld1,NPROCS_ID1,Comm_World1,NPROCS_WD1,
			     Comm_World_StartID1,MPI_CommWD1,MP,is2,ie2,
    ...
    time5 += Cluster_DFT_NonCol(command,1,SpinP_switch,
				ko_noncol,H,iHNL,OLP[0],DM[0],EDM,
				Eele0,Eele1,
				MP,is2,ie2,
				Ss_Re,Cs_Re,
  ...
  *********************************************************/
  ...
  if (Cnt_switch==1) Allocate_Free_OrbOpt(2);
  ...
  dcomplex *Hs_Cx)
{
  ...
  if (SpinP_switch==3) CDDF_approach=1; /* for non-collinear case */
  if (CDDF_approach==1) Calc_OLP_r(1,myid0); /* calculate OLPpox = <a|r|b> */
  /* YTL-New-end */
  ...
  if (myid0==Host_ID) printf("<Optical calculations end, time=%7.5f (s)>\n",Etime_CDDF-Stime_CDDF);
  /* YTL-New-start */
  if (CDDF_approach==1) free_OLP_r(); /* free OLPpox = <a|r|b> */
  /* YTL-New-end */
  ...
  ********************************************/
  ...
  if (todo_flag==1){
    ...
    ***********************************************/
    ...
    /****************************************************
        setting for BLACS in the matrix size of nxn
    ****************************************************/

    Set_BLACS_variables( MPI_CommWD2[myworld2],numprocs2,myid2,n,n,&np_rows,&np_cols,&nblk,
                         &my_prow,&my_pcol,&na_rows,&na_cols,&bhandle2,&ictxt2,
                         &na_rows_max,&na_cols_max,descS );

    for (i=0; i<9; i++){ descH[i] = descS[i]; descC[i] = descS[i]; }
...
void Allocate_Free_Band_NonCol(int todo_flag)
{
  ...
  ********************************************/
  ...
  if (todo_flag==1){
	...
	for (k=0; k<(n2+1); k++) EIGEN_Band[i][j][k] = 1.0e+5;
      }
    }
    ...
    ****************************************************/
    ...
    Hs11_Cx = (dcomplex*)malloc(sizeof(dcomplex)*na_rows*na_cols);
    Hs12_Cx = (dcomplex*)malloc(sizeof(dcomplex)*na_rows*na_cols);
    Hs22_Cx = (dcomplex*)malloc(sizeof(dcomplex)*na_rows*na_cols);
    ...
    ****************************************************/
    ...
    if (firsttime && memoryusage_fileout) {
      ...
      PrintMemory("Allocate_Free_Band_NonCol: Hs11_Cx",sizeof(dcomplex)*na_rows*na_cols,NULL);
      PrintMemory("Allocate_Free_Band_NonCol: Hs12_Cx",sizeof(dcomplex)*na_rows*na_cols,NULL);
      PrintMemory("Allocate_Free_Band_NonCol: Hs22_Cx",sizeof(dcomplex)*na_rows*na_cols,NULL);
...
void Allocate_Free_Cluster_Col(int todo_flag)
{
  ...
  int i,j,k,nblk_m,nblk_m2,wanA,spin,size_EVec1=0;
  double tmp,tmp1;
...
void Allocate_Free_Cluster_NonCol(int todo_flag)
{
  ...
  int i,j,k,nblk_m,nblk_m2,wanA,myid0,numprocs0,numprocs1;
  double tmp,tmp1;
