...
void array0()
{
  ...
  ****************************************************/
  ...
  free(Number_of_Electrons_Species);
  free(Spe_Spe2Ban);
  free(Species_Top);
  free(Species_End);
  free(F_Snd_Num);
...
/* added by mari 05.12.2014 */
  ...
  if ( atomnum<=numprocs && Solver==11 ){
    ...
    if (Num_Comm_World1_DCLNO<=numprocs0)  MPI_Comm_free(&MPI_CommWD1_DCLNO[myworld1_DCLNO]);
  ...
  if (XC_switch==4){
    ...
    free(SVals_Coulomb);
    free(SVecs_U_Coulomb);
    free(SVecs_V_Coulomb);
    free(qgrid_Rec_Coulomb);
  }
