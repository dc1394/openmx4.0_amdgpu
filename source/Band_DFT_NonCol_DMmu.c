/**********************************************************************
  Band_DFT_NonCol_DMmu.c:
     ...
     Band_DFT_NonCol_DMmu.c is a subroutine to perform band calculations
     based on a non-collinear DFT. 

  Log of Band_DFT_NonCol_DMmu.c:

     16/Feb./2019  Released by T.Ozaki
     26/Jun/2023   Modified by M. Fukuda
...
/* GPU GpuSolver context for Band_DFT_NonCol_DMmu (added by H.Kawai, Phase D) */
typedef struct {
    int                device_id;
    hipblasHandle_t     hipblas;
    hipsolverDnHandle_t gpusolver;
} BandNonColDMmuGpuSolverCtx;

static BandNonColDMmuGpuSolverCtx BandNonColDMmu_gpusolver_ctx = {0};

static void BandNonColDMmu_GpuSolver_Destroy(void)
{
    BandNonColDMmuGpuSolverCtx *ctx = &BandNonColDMmu_gpusolver_ctx;
...
static double Calc_DMmu_Band_non_collinear(
    int calc_flag,
    int store_flag,
    int myid0,
    int myid2,
                    ...
                    MPI_Comm *MPI_CommWD2)
{
  ...
  double *rDM11,*rDM22,*rDM12,*iDM12,*iDM11,*iDM22;
    ...
    for (LB_AN=0; LB_AN<=FNAN[GA_AN]; LB_AN++){
      ...
      for (i=0; i<tnoA; i++){
        for (j=0; j<tnoB; j++){
          ...
          iCDM[0][MA_AN][LB_AN][i][j] = 0.0;
          iCDM[1][MA_AN][LB_AN][i][j] = 0.0;
        }
      }
    }
  }
    ...
    if (SCF_iter==1 || all_knum!=1){
      ...
      if (kloop0<num_kloop0){
        ...
        if (scf_eigen_lib_flag==1){
          ...
          OpenMX_GpuSolver_DenseZheevx_1based(Cs,Ss,ko,n,n,"Band_DFT_NonCol_DMmu overlap GPU solver");
        }
    ...
    if (kloop0<num_kloop0){
      ...
      */
      ...
      if (scf_eigen_lib_flag==1){
        ...
        OpenMX_GpuSolver_DenseZheevx_1based(Hs2,Cs2,ko,n2,MaxN,"Band_DFT_NonCol_DMmu Hamiltonian GPU solver");
      }
  ...
  **************************************/
  ...
  /****************************************************
       ...
       iCDM[0]  Im alpha alpha density matrix
       iCDM[1]  Im beta  beta  density matrix
  ...
  ****************************************************/
  ...
  if (all_knum==1){
    ...
    /* calculate DM, iDM  */
    Calc_DMmu_Band_non_collinear( 1,1,
                                myid0,myid2,size_H1,
                                is2,ie2,MP,n,n2,MaxN,k1,k2,k3, 
                                CDM,iCDM,EIGEN[0][kloop],
                                EVec1[0],
  ...
  ****************************************************/
  ...
  if (all_knum!=1){
    ...
    for (kloop0=0; kloop0<max_num_kloop0; kloop0++){
      ...
      if (kloop0<num_kloop0){
        ...
        if (scf_eigen_lib_flag==1){
          ...
          OpenMX_GpuSolver_DenseZheevx_1based(Cs,Ss,ko,n,n,"Band_DFT_NonCol_DMmu DM overlap GPU solver");
        }
	...
	Hs12[i] = Complex(0.0,0.0);
      ...
      if (kloop0<num_kloop0){
        ...
        if (scf_eigen_lib_flag==1){
          ...
          OpenMX_GpuSolver_DenseZheevx_1based(Hs2,Cs2,ko,n2,MaxN,"Band_DFT_NonCol_DMmu DM Hamiltonian GPU solver");
        }
      ...
      ****************************************************/
      ...
      Calc_DMmu_Band_non_collinear( (kloop0<num_kloop0),0,
                                  myid0,myid2,size_H1,
                                  is2,ie2,MP,n,n2,MaxN,k1,k2,k3, 
                                  CDM,iCDM,EIGEN[0][kloop],
                                  EVec1[0],
    ...
    Calc_DMmu_Band_non_collinear( 0,1,
                                myid0,myid2,size_H1,
                                is2,ie2,MP,n,n2,MaxN,k1,k2,k3, 
                                CDM,iCDM,EIGEN[0][kloop],
                                EVec1[0],
    ...
    for (LB_AN=0; LB_AN<=FNAN[GA_AN]; LB_AN++){
      ...
      for (i=0; i<tnoA; i++){
        for (j=0; j<tnoB; j++){
          ...
          iCDM[0][MA_AN][LB_AN][i][j] = iCDM[0][MA_AN][LB_AN][i][j]*dum;
          iCDM[1][MA_AN][LB_AN][i][j] = iCDM[1][MA_AN][LB_AN][i][j]*dum;
        }
      }
    }
  }
...
double Calc_DMmu_Band_non_collinear(
    int calc_flag,
    int store_flag,
    int myid0,
    int myid2,
  ...
  if (calc_flag==1){
    ...
    for (k=is2[myid2]; k<=ie2[myid2]; k++){
      ...
      if ((flag_export_DM==1) && (flag_energy_range_DM==1) ){
        if((eig - ChemP < DM_energy_range[0])||(DM_energy_range[1] < eig - ChemP)){
            tmp1 = 0.0;
        }
      }
        ...
        FermiF = FermiF_upper;
        tmp1 = sqrt(FermiF_upper - FermiF_lower);
      }
      /* */
