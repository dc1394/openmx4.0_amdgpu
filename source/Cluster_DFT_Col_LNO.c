...
/* GPU GpuSolver context for Cluster_DFT_Col_LNO (added by H.Kawai, Phase D) */
typedef struct {
    int                device_id;
    hipblasHandle_t     hipblas;
    hipsolverDnHandle_t gpusolver;
} ClusterColLNOGpuSolverCtx;

static ClusterColLNOGpuSolverCtx ClusterColLNO_gpusolver_ctx = {0};

static void ClusterColLNO_GpuSolver_Destroy(void)
{
    ClusterColLNOGpuSolverCtx *ctx = &ClusterColLNO_gpusolver_ctx;
...
static void ClusterColLNO_GpuSolver_Init(void)
{
    ClusterColLNOGpuSolverCtx *ctx = &ClusterColLNO_gpusolver_ctx;
    int current_device;
    wait_hipfunc(hipGetDevice(&current_device));
    if (ctx->device_id == current_device && ctx->hipblas != NULL) return;
    if (ctx->hipblas != NULL) ClusterColLNO_GpuSolver_Destroy();
    ctx->device_id = current_device;
    wait_hipfunc(hipblasCreate(&ctx->hipblas));
    wait_hipfunc(hipsolverDnCreate(&ctx->gpusolver));
}

static void ClusterColLNO_GEMMul8Dgemm_OpenMP(hipblasOperation_t transa, hipblasOperation_t transb, int m, int n, int k,
                                               double const * A, double const * B, double * C)
{
    ClusterColLNO_GpuSolver_Init();
    {
        double const alpha = 1.0;
        double const beta  = 0.0;
        wait_hipfunc(openmx_gemmul8Dgemm(ClusterColLNO_gpusolver_ctx.hipblas, transa, transb, m, n, k, &alpha, A, m, B, k, &beta, C, m));
    }
}
                   ...
                   double *Work1)
{
 ...
 S_diagonalize:
    ...
    else if (scf_eigen_lib_flag==GPUSOLVER && GPU_CPU_SWITCH_NUM<=TNum_LNOs && na_rows==TNum_LNOs && na_cols==TNum_LNOs){
      OpenMX_GpuSolver_DenseDsyevx_1based(Cs,Ss_LNO[spin],ko[0],TNum_LNOs,TNum_LNOs,
                                         "Cluster_DFT_Col_LNO overlap GPU solver");
    }
 ...
 diagonalize:
  ...
  if (scf_eigen_lib_flag==1){
    ...
    OpenMX_GpuSolver_DenseDsyevx_1based(Hs,Cs,ko[spin],TNum_LNOs,MaxN,
                                       "Cluster_DFT_Col_LNO Hamiltonian GPU solver");
  }
