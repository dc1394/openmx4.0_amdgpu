...
static int SOG_gpu_eligible(int Cnt_kind)
{
  ...
  if (!device_checked) {
    device_checked = 1;
    /* Probe the context and OpenMP target module, not just the HIP device
       count: a rank that cannot finish initialization must use the host. */
...
#pragma omp target teams distribute parallel for thread_limit(128) is_device_ptr(nog, ppr, pairs, gla, cla, atvf, rv_all, rwf_all, rv_off, rwf_base, \
...
static int Set_Orbitals_Grid_GPU(void)
{
  ...
  hipMemcpy(arena + o_rv, pao_rv, sizeof(double) * (rv_cnt == 0 ? 1 : rv_cnt),hipMemcpyHostToDevice);
  hipMemcpy(arena + o_rwf, pao_rwf, sizeof(double) * (rwf_cnt == 0 ? 1 : rwf_cnt),hipMemcpyHostToDevice);
  hipMemcpy(arena + o_rvo, rv_off_h, sizeof(size_t) * (size_t)(SpeciesNum + 1),hipMemcpyHostToDevice);
  hipMemcpy(arena + o_rwb, rwf_base_h, sizeof(size_t) * (size_t)(SpeciesNum + 1),hipMemcpyHostToDevice);
  hipMemcpy(arena + o_spm, sp_mesh_h, sizeof(int) * (size_t)SpeciesNum,hipMemcpyHostToDevice);
  hipMemcpy(arena + o_spx, sp_maxl_h, sizeof(int) * (size_t)SpeciesNum,hipMemcpyHostToDevice);
  hipMemcpy(arena + o_spb, sp_nb_h, sizeof(int) * (size_t)SpeciesNum * (SOG_L0MAX + 1),hipMemcpyHostToDevice);
  hipMemcpy(arena + o_atv, atv_flat, sizeof(double) * atv_rows * 3,hipMemcpyHostToDevice);

  {
    ...
    for (Mc_AN = 1; Mc_AN <= Matomnum; Mc_AN++) {
      ...
      if (0 < npts) {
        ...
        hipMemcpy((void*)d_gla, GridListAtom[Mc_AN], sizeof(int) * (size_t)npts,hipMemcpyHostToDevice);
        hipMemcpy((void*)d_cla, CellListAtom[Mc_AN], sizeof(int) * (size_t)npts,hipMemcpyHostToDevice);
        ...
        pair_host[0].ay = 0.0;
        pair_host[0].az = 0.0;
        hipMemcpy((void*)d_prs, pair_host, sizeof(SOG_GpuPair),hipMemcpyHostToDevice);
        ...
        hipMemcpy(chi_host, d_chi, sizeof(float) * (size_t)npts * (size_t)NO0,hipMemcpyDeviceToHost);
...
#pragma omp parallel for private(i)
      ...
      for (h_AN = 0; h_AN <= FNAN[Gc_AN]; h_AN++) {
        ...
        if (G2ID[Gh_AN] != myid && 0 < NumOLG[Mc_AN][h_AN]) {
          ...
          hipMemcpy((void*)(d_nog + fpts), GListTAtoms1[Mc_AN][h_AN],
                               sizeof(int) * (size_t)nolg,hipMemcpyHostToDevice);
      ...
      if (0 < npair) {
        ...
        hipMemcpy((void*)d_ppr, ppr_host, sizeof(int) * fpts,hipMemcpyHostToDevice);
        hipMemcpy((void*)d_prs, pair_host, sizeof(SOG_GpuPair) * (size_t)npair,hipMemcpyHostToDevice);
        ...
        hipMemcpy(chi_host, d_chi, sizeof(float) * fchi,hipMemcpyDeviceToHost);
