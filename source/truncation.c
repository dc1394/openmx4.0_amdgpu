...
{ 
  ...
  ****************************************************/
  ...
  if (Wannier90_fileout==7 || Wannier90_fileout==8){

    ct_AN = 1;
    ID = G2ID[ct_AN];
    printf("FFF1 myid=%2d Wannier90_fileout=%2d ct_AN=%2d ID=%2d\n",
            myid,Wannier90_fileout,ct_AN,ID);
    ...
    exit(0);
      ...
      /*
  if (Wannier90_fileout==7 || Wannier90_fileout==8){
    printf("GGG1 Wannier90_fileout=%2d\n",Wannier90_fileout); 

    int tmp=myid+1;
    MPI_Bcast(&tmp, 1, MPI_INT, 0, mpi_comm_level1);
    printf("WWW1 %2d %2d\n",Wannier90_fileout,tmp);
    MPI_Finalize();
    exit(0);

    printf("FFF1 Wannier90_fileout=%2d ct_AN=%2d ID=%2d\n",Wannier90_fileout,ct_AN,ID);
  ...
  ****************************************************/
  ...
  if (MD_iter==1 && UCell_flag==1){
    ...
    *************************************/
    ...
    if (MYID_MPI_COMM_WORLD==Host_ID && 0<level_stdout)  printf("<UCell_Box> Info. of cutoff energy and num. of grids\n");
  ...
  ****************************************************/
  ...
  if (CWF_Calc==1 && CWF_Guiding_Orbital==3){
    ...
    ParDM = (double*****)malloc(sizeof(double****)*(SpinP_switch+1)); 
    for (k=0; k<=SpinP_switch; k++){
      ParDM[k] = (double****)malloc(sizeof(double***)*(Matomnum+1)); 
      FNAN[0] = 0;
      for (Mc_AN=0; Mc_AN<=Matomnum; Mc_AN++){
	...
	ParDM[k][Mc_AN] = (double***)malloc(sizeof(double**)*(FNAN[Gc_AN]+1));
	for (h_AN=0; h_AN<=FNAN[Gc_AN]; h_AN++){
	  ...
	  ParDM[k][Mc_AN][h_AN] = (double**)malloc(sizeof(double*)*tno0); 
	  for (i=0; i<tno0; i++){
	    ParDM[k][Mc_AN][h_AN][i] = (double*)malloc(sizeof(double)*tno1); 
	    for (j=0; j<tno1; j++) ParDM[k][Mc_AN][h_AN][i][j] = 0.0; 
	  }
	}
      }
    }
    ...
    if (SpinP_switch==3){
      ...
      iParDM = (double*****)malloc(sizeof(double****)*2); 
      for (k=0; k<2; k++){
	iParDM[k] = (double****)malloc(sizeof(double***)*(Matomnum+1)); 
	FNAN[0] = 0;
	for (Mc_AN=0; Mc_AN<=Matomnum; Mc_AN++){
	  ...
	  iParDM[k][Mc_AN] = (double***)malloc(sizeof(double**)*(FNAN[Gc_AN]+1));
	  for (h_AN=0; h_AN<=FNAN[Gc_AN]; h_AN++){
	    ...
	    iParDM[k][Mc_AN][h_AN] = (double**)malloc(sizeof(double*)*tno0); 
	    for (i=0; i<tno0; i++){
	      iParDM[k][Mc_AN][h_AN][i] = (double*)malloc(sizeof(double)*tno1); 
	      for (j=0; j<tno1; j++) iParDM[k][Mc_AN][h_AN][i][j] = 0.0; 
	    }
	  }
	}
      }
    } // end of if (SpinP_switch==3)
  }
  ...
  if (Solver==8) { /* Krylov subspace method */
	...
	Krylov_U[i][j] = (double*)malloc(sizeof(double)*rlmax_EC[j]*EKC_core_size[j]*csize);
      ...
      if (i==0){
        ...
        if (MYID_MPI_COMM_WORLD==Host_ID && 2<=level_stdout){
...
#pragma omp parallel shared(myid,ScaleSize,Max_FSNAN,level_stdout,BCR,atv,Gxyz,Dis,ncn,natn,TCpyCell,CpyCell,atomnum,FNAN,SNAN,Spe_Atom_Cut1,WhatSpecies,M2G,Matomnum) private(OMPID,Nthrds,Nprocs,i,ct_AN,wanA,rcutA,j,wanB,rcutB,rcut,Rn,dx,dy,dz,r,l,k,fDis,fncn,fnan2,sDis,sncn,snan2,size_array,rcut_max)
  ...
  ****************************************************/
  ...
  if (Wannier90_fileout==7 || Wannier90_fileout==8){
    ...
    int tmp=myid+1;
    MPI_Bcast(&FNAN[1], 1, MPI_INT, 0, mpi_comm_level1);
    printf("WWW1 %2d %2d\n",Wannier90_fileout,tmp);
    MPI_Finalize();
    exit(0);
  }  
...
void Check_System()
{
  ...
  if (MYID_MPI_COMM_WORLD==Host_ID && 0<level_stdout) printf("<Check_System> The system is %s\n",s_vec[num]);
    ...
    ************/
    ...
    if ( 1.0e+8<scf_fixed_origin[0] &&
	 ...
	 1.0e+8<scf_fixed_origin[2] ){
      ...
      /* added by T.Ohwaki */
      if (MYID_MPI_COMM_WORLD==Host_ID && ESM_switch!=0 && 0<level_stdout){
...
#pragma omp parallel shared(level_stdout,rtv,Cell_Gxyz,Grid_Origin,Gxyz,M2G,Matomnum) private(Mc_AN,Gc_AN,OMPID,Nthrds,Nprocs)
  ...
  ****************************************************/
  ...
  if ( (estimate_switch==0 || 2<=level_stdout) && MYID_MPI_COMM_WORLD==Host_ID ){
  ...
  ****************************************************/
  ...
  if (MYID_MPI_COMM_WORLD==Host_ID && estimate_switch==0 && 0<level_stdout){
    for (ct_AN=1; ct_AN<=atomnum; ct_AN++){
      if (ct_AN<=20 && level_stdout<=1){
  ...
  if (alloc_first[4]==0){
	...
	for (Mc_AN=0; Mc_AN<=Matomnum; Mc_AN++){
	  ...
	  for (h_AN=0; h_AN<=FNAN[Gc_AN]; h_AN++){
	    ...
	    for (i=0; i<tno0; i++){
	      free(ParDM[k][Mc_AN][h_AN][i]);
	    }
            free(ParDM[k][Mc_AN][h_AN]);
	  }
          free(ParDM[k][Mc_AN]);
	}
        free(ParDM[k]);
	...
	for (k=0; k<2; k++){
	  ...
	  for (Mc_AN=0; Mc_AN<=Matomnum; Mc_AN++){
		...
		tno1 = Spe_Total_NO[Hwan];
	      ...
	      for (i=0; i<tno0; i++){
		free(iParDM[k][Mc_AN][h_AN][i]);
	      }
 	      free(iParDM[k][Mc_AN][h_AN]);
	    }
            free(iParDM[k][Mc_AN]);
	  }
          free(iParDM[k]);
  ...
  *************************************************/
  ...
  for (ID=0; ID<numprocs; ID++){ 
    ...
    for (i=0; i<Num_Pro_Snd[ID]; i++){
       ...
       if (po==2){
         if (MYID_MPI_COMM_WORLD==Host_ID ) printf("Technical error in truncation.c: error code 10.\n");
         MPI_Finalize();
         exit(0); 
       }
