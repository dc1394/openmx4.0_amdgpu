		   ...
		   double ******ko)
{
  ...
  *****************************************/
  ...
  if (*mode==5){
    ...
    *WhatSpecies = (int*)malloc(sizeof(int)*(*atomnum));
    if ( (fp=input_find("<WhatSpecies")) != NULL ) {
      for (i=0;i<*atomnum;i++) {
    ...
    *Spe_Total_CNO= (int*)malloc(sizeof(int)*(*SpeciesNum));
    if ( (fp=input_find("<Spe_Total_CNO")) != NULL ) {
      for (i=0;i<*SpeciesNum;i++) {
    ...
    *Spe_Num_CBasis=(int**)malloc_multidimarray("int",N_Spe_Num_CBasis, i_Spe_Num_CBasis);
    if ( (fp=input_find("<Spe_Num_CBasis")) != NULL ) {
      for (i=0;i<*SpeciesNum;i++) {
	for (j=0;j<=*MaxL;j++) {
    ...
    *Spe_Num_Relation=(int**)malloc_multidimarray("int",N_Spe_Num_Relation, i_Spe_Num_Relation);
    ...
    if (*NonCol==1){
      ...
      *Angle1_Spin = (double*)malloc(sizeof(double)*(*atomnum));
      if ( (fp=input_find("<SpinAngle")) !=NULL ) {
        for (i=0; i<*atomnum; i++) {
  ...
  *****************************************/
    ...
    *Spe_Num_Relation=(int**)malloc_multidimarray("int",N_Spe_Num_Relation, i_Spe_Num_Relation);
    ...
    if ( (fp=input_find("<Eigenvalues")) != NULL ) {
      N_ko=5; 
	...
	exit(10);
      ...
      printf("cannot find <Eigenvalues\n");
      exit(10);
    }
  }
