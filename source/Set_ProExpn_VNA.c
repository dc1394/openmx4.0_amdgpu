...
/* Device bytes the "#pragma omp target data" region in SetPro_DSVNA_GpuRun
   will map.  Deliberately mirrors that clause term for term, in the same
                                 ...
                                 double ****Bessel_Pro00, double ****Bessel_Pro01)
{
  ...
  /* Cheap gate only.  The real device-memory decision cannot be made here --
     every extent the omp target data region is sized from is still unknown --
     so it happens at the end of this function, once the tables are built. */
      ...
      for (M0=-L0; M0<=L0; M0++){
        for (L1=0; L1<nL1; L1++){
          for (M1=-L1; M1<=L1; M1++){
            for (LL=0; LL<nLL; LL++){
              for (m=-LL; m<=LL; m++){
                if (M1==M0-m && __builtin_abs(L1-LL)<=L0 && L0<=(L1+LL)){
  ...
  /* Device-memory decision.  It has to be here rather than on entry: cmb_max,
     ...
     above, and the omp target data region in SetPro_DSVNA_GpuRun is sized
     entirely from them.  The check this replaces ran on entry against a flat
     1 GiB placeholder, so a large run could walk into a multi-GiB device
     mapping and the OpenMP runtime aborted every rank -- there is no way to
...
#pragma omp target data map(to:pro00[0:pro_len], pro01[0:pro_len], vnab[0:vnab_len], \
                        ...
                        GL_NormK[0:GL_Mesh]) \
                 map(alloc:sphb[0:sphb_len], sphbp[0:sphb_len], \
...
#pragma omp target update to(pr_r[0:np], pr_siT[0:np], pr_coT[0:np], \
      ...
      for (size_t e=0; e<(size_t)np*elem_per_pair; e++){
        ...
        if (L0>=0 && M0>=-L0 && M0<=L0 && M1>=-L1 && M1<=L1){
          ...
          for (LL=0; LL<=lfi; LL++){
            ...
            if ( __builtin_abs(L1-LL)<=L0 && L0<=(L1+LL) ){
...
#pragma omp target update from(outbuf[0:(size_t)np*4*(size_t)mn_max])
            ...
            for (LL=0; LL<nLL; LL++){
              ...
              if (-LL<=m && m<=LL){
                size_t idx = (((((size_t)L0v*nM + (M0+g->l0max))*(g->l0max+1) + L1v)*nM
                               + (M1+g->l0max))*nLL + LL);
                g->gnt2[idx] = pow(-1.0,(double)__builtin_abs(m))*Gaunt(L0v,M0,L1v,M1,LL,-m);
              }
            }
          }
        }
...
#pragma omp target data map(to:crude[0:crude_len], prod[0:prod_len], gnt2[0:gnt2_len], \
...
#pragma omp target teams distribute parallel for
      for (size_t q=0; q<(size_t)np*nLL*cmb_max*cmb_max; q++){
        ...
        if (L0>=0 && L1>=0 && L0<=L1 && LLq<=2*L1 &&
            __builtin_abs(L1-LLq)<=L0 && L0<=(L1+LLq)){
      ...
      for (size_t e=0; e<(size_t)np*elem_per_pair; e++){
        ...
        if (L0>=0 && L1>=0 && L0<=L1 &&
          ...
          for (LL=0; LL<=2*L1; LL++){
            ...
            if (__builtin_abs(L1-LL)<=L0 && L0<=(L1+LL) && -LL<=m && m<=LL){
		  ...
		  if (__builtin_abs(L1-LL)<=L0 && L0<=(L1+LL) ){
			...
			for(m=-LL; m<=LL; m++){ 
			  ...
			  if ( (M1-m)==M0){
			    ...
			    gant = pow(-1.0,(double)__builtin_abs(m))*Gaunt(L0,M0,L1,M1,LL,-m);
