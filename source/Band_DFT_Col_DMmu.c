/**********************************************************************
  Band_DFT_Col_DMmu.c:
     ...
     Band_DFT_Col_DMmu.c is a subroutine to perform band calculations
     based on a collinear DFT

  Log of Band_DFT_Col_DMmu.c:

     22/Nov/2001  Released by T. Ozaki
     30/May/2023   Modified by M. Fukuda
...
/* GPU GpuSolver context for Band_DFT_Col_DMmu (added by H.Kawai, Phase D) */
typedef struct {
    int                device_id;
    hipblasHandle_t     hipblas;
    hipsolverDnHandle_t gpusolver;
} BandColDMmuGpuSolverCtx;

static BandColDMmuGpuSolverCtx BandColDMmu_gpusolver_ctx = {0};

static void BandColDMmu_GpuSolver_Destroy(void)
{
    BandColDMmuGpuSolverCtx *ctx = &BandColDMmu_gpusolver_ctx;
...
diagonalize1:
  ...
  ****************************************************/
  ...
  if (all_knum==1){
    ...
    /* sum of CDM1 by Allreduce in MPI */
    ...
    /* store DM1 to a proper place in CDM */
