...
/* GPU CuSolver context for Band_DFT_Col_DMmu (added by H.Kawai, Phase D) */
typedef struct {
    int                device_id;
    cublasHandle_t     cublas;
    cusolverDnHandle_t cusolver;
} BandColDMmuCuSolverCtx;

static BandColDMmuCuSolverCtx BandColDMmu_cusolver_ctx = {0};

static void BandColDMmu_CuSolver_Destroy(void)
{
    BandColDMmuCuSolverCtx *ctx = &BandColDMmu_cusolver_ctx;
...
#pragma acc host_data use_device(A, B, C)
    {
        ...
        wait_cudafunc(openmx_gemmul8Zgemm(BandColDMmu_cusolver_ctx.cublas, transa, transb, m, n, k, &alpha,
