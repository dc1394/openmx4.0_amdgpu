/*
    -- MAGMA (version 2.9.0) --
       ...
       @generated from src/zgetri_gpu.cpp, normal z -> s, Wed Jan 22 14:39:10 2025
    ...
    magma_queue_t queues[2] )
{
    ...
    magma_int_t lwork_device_getri = n * nb * sizeof(float);
    ...
    magma_strtri_expert_gpu_work(MagmaUpper, MagmaNonUnit, n, NULL, ldda, info, NULL, lwork_host_trtri, NULL, lwork_device_trtri, NULL);
    ...
    *info = 0;
    ...
    magma_int_t lwork = lwork_device_getri / sizeof(float);
    if (lwork >= ldda*n) {
        lddl = ldda;
    }
    else {
    ...
    trtri_host_work = (magmaFloat_ptr)host_work;

    // assign device pointers
    void *trtri_device_work=NULL;
    dL = (magmaFloat_ptr)device_work;
    trtri_device_work = (void*)(dL + (lwork_device_getri/sizeof(float)) );
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magmablas_slaset( MagmaLower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda, queues[0] );
    ...
    for( j = n-2; j >= 0; --j ) {
        jp = ipiv[j] - 1;
        if ( jp != j ) {
            magmablas_sswap( n, dA(0,j), 1, dA(0,jp), 1, queues[0] );
        }
    }
...
magma_sgetri_gpu(
    ...
    magma_int_t *info )
{
    magma_int_t lwork_bytes = lwork * sizeof(float);
