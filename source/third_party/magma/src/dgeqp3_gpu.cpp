/*
    -- MAGMA (version 2.9.0) --
       ...
       @generated from src/zgeqp3_gpu.cpp, normal z -> d, Wed Jan 22 14:39:17 2025
    ...
    magma_int_t *info, magma_queue_t queue )
{
    ...
    d_workspace_bytes += lwkopt     * sizeof(double);
    d_workspace_bytes += (n+1) * nb * sizeof(double); // df
    d_workspace_bytes += ( 1+256*(n+255)/256 ) * sizeof(double);  // dlsticcs

    // round up to multilpe of sizeof(double)
    // this is potentially important for the top-level interface, which uses lwork
    // as the number of elements, not bytes
    d_workspace_bytes = magma_roundup(d_workspace_bytes, sizeof(double));
    ...
    *info = 0;
    ...
    // assign pointers
    magmaDouble_ptr dwork=NULL, df=NULL;
    double* dlsticcs=NULL;
    dwork    = (magmaDouble_ptr)device_work;
    df       = dwork + (n + 1) * nb;
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    else if( lwork*sizeof(double) < lwork_device[0] ) {
        *info = -8;
    }
