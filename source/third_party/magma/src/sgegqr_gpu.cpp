/*
    -- MAGMA (version 2.9.0) --
       ...
       @author Stan Tomov
       @generated from src/zgegqr_gpu.cpp, normal z -> s, Wed Jan 22 14:39:14 2025
...
/***************************************************************************//**
    ...
    host_work  CPU workspace, size determined by lwork_host
               On exit, the first n^2 REAL elements hold the rectangular
               matrix R.
    ...
    magma_int_t *info, magma_queue_t queue )
{
    ...
    magma_int_t h_workspace_bytes = 3 * n * n * sizeof(float);
    if(ikind == 1) {
        h_workspace_bytes += (32 + 2*n*n + 2*n) * sizeof(float);
        #ifdef COMPLEX
    ...
    switch( ikind ) {
        case  1: d_workspace_bytes = sizeof(float) * n * n; break;
        case  2: d_workspace_bytes = sizeof(float) * (3 * n * n + min(m, n) + 2); break;
        case  4: d_workspace_bytes = sizeof(float) * n * n; break;
    ...
    *info = 0;
    ...
    // assign pointers
    magmaFloat_ptr dwork = (magmaFloat_ptr)device_work;
    magmaFloat_ptr work  = (magmaFloat_ptr)host_work;
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    magma_sgegqr_expert_gpu_work( ikind, m, n, NULL, ldda, NULL, lwork_host, NULL, lwork_device, info, NULL );
