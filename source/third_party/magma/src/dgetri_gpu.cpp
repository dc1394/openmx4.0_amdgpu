/*
    -- MAGMA (version 2.9.0) --
       ...
       @generated from src/zgetri_gpu.cpp, normal z -> d, Wed Jan 22 14:39:10 2025
    ...
    magma_queue_t queues[2] )
{
    ...
    magma_int_t lwork_device_getri = n * nb * sizeof(double);
    ...
    magma_dtrtri_expert_gpu_work(MagmaUpper, MagmaNonUnit, n, NULL, ldda, info, NULL, lwork_host_trtri, NULL, lwork_device_trtri, NULL);
    ...
    *info = 0;
    ...
    magma_int_t lwork = lwork_device_getri / sizeof(double);
    if (lwork >= ldda*n) {
        lddl = ldda;
    }
    else {
    ...
    trtri_host_work = (magmaDouble_ptr)host_work;

    // assign device pointers
    void *trtri_device_work=NULL;
    dL = (magmaDouble_ptr)device_work;
    trtri_device_work = (void*)(dL + (lwork_device_getri/sizeof(double)) );
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magmablas_dlaset( MagmaLower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda, queues[0] );
...
magma_dgetri_gpu(
    ...
    magma_int_t *info )
{
    magma_int_t lwork_bytes = lwork * sizeof(double);
