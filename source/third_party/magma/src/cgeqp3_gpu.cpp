/*
    -- MAGMA (version 2.9.0) --
       ...
       @generated from src/zgeqp3_gpu.cpp, normal z -> c, Wed Jan 22 14:39:17 2025
    ...
    magma_int_t *info, magma_queue_t queue )
{
    ...
    d_workspace_bytes += lwkopt     * sizeof(magmaFloatComplex);
    d_workspace_bytes += (n+1) * nb * sizeof(magmaFloatComplex); // df
    d_workspace_bytes += ( 1+256*(n+255)/256 ) * sizeof(float);  // dlsticcs

    // round up to multilpe of sizeof(magmaFloatComplex)
    // this is potentially important for the top-level interface, which uses lwork
    // as the number of elements, not bytes
    d_workspace_bytes = magma_roundup(d_workspace_bytes, sizeof(magmaFloatComplex));
    ...
    *info = 0;
    ...
    // assign pointers
    magmaFloatComplex_ptr dwork=NULL, df=NULL;
    float* dlsticcs=NULL;
    dwork    = (magmaFloatComplex_ptr)device_work;
    df       = dwork + (n + 1) * nb;
    dlsticcs = (float*)(df    + (n + 1) * nb);
    float *rwork = dlsticcs + (1+256*((n+255)/256));
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    else if( lwork*sizeof(magmaFloatComplex) < lwork_device[0] ) {
        *info = -8;
    }
