/*
    -- MAGMA (version 2.9.0) --
       ...
       @generated from testing/testing_zgegqr_gpu.cpp, normal z -> s, Wed Jan 22 14:40:31 2025
       @author Stan Tomov
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lwork  = max( lwork,  magma_ceildiv(gegqr_lhwork[0], sizeof(float)) );
            ldwork = max( ldwork, magma_ceildiv(gegqr_ldwork[0], sizeof(float)) );

            // update  gegqr_lhwork & gegqr_ldwork
            gegqr_lhwork[0] = lwork  * sizeof(float);
            gegqr_ldwork[0] = ldwork * sizeof(float);
