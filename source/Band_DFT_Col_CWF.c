...
static void BandColCWF_GpuSolver_Init(void)
{
    ...
    if (ctx->cublas != NULL) BandColCWF_GpuSolver_Destroy();
    ctx->device_id = current_device;
    wait_cudafunc(cublasCreate(&ctx->cublas));
    wait_cudafunc(cusolverDnCreate(&ctx->gpusolver));
}

static void BandColCWF_GEMMul8Zgemm_OpenACC(cublasOperation_t transa, cublasOperation_t transb, int m, int n, int k,
...
#pragma acc host_data use_device(A, B, C)
    {
        ...
        wait_cudafunc(openmx_gemmul8Zgemm(BandColCWF_gpusolver_ctx.cublas, transa, transb, m, n, k, &alpha,
