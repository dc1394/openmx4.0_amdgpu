...
void Input_std(char *file)
{
  ...
  ****************************************************/
  ...
  input_double("scf.energycutoff",&Grid_Ecut,(double)220.0);
  ...
  ****************************************************/
  ...
  s_vec[0]="GCenter"; s_vec[1]="none"; s_vec[2]="REGULAR";
  ...
  if (CWF_Calc==1 && way_of_kpoint!=3){
    if (myid==Host_ID){
      printf("For CWF calculation, only GCenter for scf.Generation.Kpoint is supported.\n");
      printf("Check your input file.\n\n");
    }
    MPI_Finalize();
    exit(0);
  ...
  /* In case of CWF_Calc==1, please check Solver */

  if (CWF_Calc==1 && !(Solver==2 || Solver==3) ){
    if (myid==Host_ID){
      printf("Fo the CWF calculation, only Cluster and Band are supported for scf.EigenvalueSolver.\n");
      printf("Check your input file.\n\n");
    }
    MPI_Finalize();
    exit(0);
  ...
  ****************************************************/
  ...
  s_vec[0]="LDA"; s_vec[1]="LSDA-CA"; s_vec[2]="LSDA-PW"; s_vec[3]="GGA-PBE"; s_vec[4]="HSE"; s_vec[5]="NLX";
  ...
  *************************************************************************/
  ...
  input_logical("CDDF.analysis",&CDDF_analysis,0); /* default=off */
  input_double("CDDF.analysis.energy",&CDDF_analysis_energy,3.0);  /* default=3.0 eV */
  input_double("CDDF.analysis.cutoff",&CDDF_analysis_cutoff,10.0); /* default= 10.0 a.u. */
  ...
  *********************************************************/
  ...
  if (myid==Host_ID && 0<level_stdout){
    ...
    if (ESM_switch==1){
      ...
      printf("                                                           \n");
      printf("        Copyright (C), 2011-2026, T.Ohwaki and M.Otani     \n");
      printf("********************************************************** \n");
      printf("\n");
    }
  ...
  ************************************************************************/
  ...
  s_vec[0]="Poly";    i_vec[0]=0;
  s_vec[1]="Fermi";   i_vec[1]=1; 
  input_string2int("CWF.window.function",&CWF_WindowF_switch,2,s_vec,i_vec);

  input_logical("CWF.Dis.vs.H",&CWF_Dis_vs_H,0); /* default=off */

  if (CWF_WindowF_switch==0){
    r_vec[0]=-1000.0; r_vec[1]=-100.0; r_vec[2] = 0.0; r_vec[3] = 3.0;
    input_doublev("CWF.disentangling.Erange",4,CWF_disentangling_Erange,r_vec);
    /* change the unit from eV to Hartree */
    ...
    CWF_disentangling_Erange[2]= CWF_disentangling_Erange[2]/eV2Hartree;
    CWF_disentangling_Erange[3]= CWF_disentangling_Erange[3]/eV2Hartree;
  }
    ...
    input_double("CWF.disentangling.smearing.bound",&CWF_disentangling_smearing_bound,(double)1.0e-10);
  }
  ...
  if ( CWF_Calc==1 && CWF_Guiding_Orbital==3 ){
	...
	fscanf(fp,"%d %d",&j,&num);
	  ...
	  po++;
        ...
        if (num<0){
          if (myid==Host_ID){
  	    printf("The number of guiding functions should not be negative\n");
	  }
	  po++;
	}
  ...
  input_logical("CWF.Wannier90.fileout",&cwf2w90_output_flag,0);
  ...
  if (cwf2w90_output_flag 
    ...
    if (myid==Host_ID){
      printf("CWF.Wannier90.fileout=on is not supported for the gamma point calculation.\n");fflush(stdout);
    }
  ...
  **************************************************/
  ...
  if ( CWF_Calc==1 ){
	...
	for (gidx=0; gidx<Num_CWF_Grouped_Atoms; gidx++){
      ...
      else if (CWF_Guiding_Orbital==4){
        /* TNum_CWFs is set tempory */
        /* TNum_CWFs will be reset in SetPara_DFT.c */
	TNum_CWFs = 30*atomnum;
      }
    }
  ...
  *****************************************************/
  ...
  s_vec[4]="pp";       i_vec[4] = 3;   /* post-processing  */
  ...
  if (Scf_RestartFromFile==-1){
    Scf_RestartToFile = 0;   /* no-updatign restart files */
  }
  else if (Scf_RestartFromFile==0){
    Scf_RestartToFile = 1;   /* updatign restart files */
  }
  else if (Scf_RestartFromFile==1){
    Scf_RestartToFile = 1;   /* updatign restart files */
  }
  else if (Scf_RestartFromFile==2){
    Scf_RestartToFile = 0;   /* no-updating restart files */
  }
  else if (Scf_RestartFromFile==3){
  ...
  ****************************************************/
  ...
  /* for arbitrary path choice as same as the electronic band path */
  {
    ...
    if (UnfoldBand_Nkpath>0) {
      ...
      if ( (fp=input_find("<UnfoldBand.kpath")) !=NULL ) {
  ...
  /* added by M. FUKUDA for export Density Matrix for FLPQ*/
  /****************************************************
               Density Matrix
  ****************************************************/
  ...
  /***********************************************
           manually specified k-points 
  ***********************************************/
  s_vec[0]="Off"; s_vec[1]="On"; s_vec[2]="window"; s_vec[3]="mu";
  ...
  ***********************************************/
  input_logical("DM.specify.energy.range",&flag_energy_range_DM,0);
  
  if (flag_export_DM>0){
    r_vec[0]=-10.0; r_vec[1]=10.0;
    input_doublev("DM.energy.range",2,DM_energy_range,r_vec);
    DM_energy_range[0] = DM_energy_range[0]/eV2Hartree;
  ...
  **************************************************/
  ...
  if (COHP_calc_flag==1){
    ...
    if (Dos_fileout==0){
      if (myid==Host_ID) printf("In case of the COHP calculation, Dos.fileout should be switched on.\n");
      po++;
    }
	...
	/* format error */
	if (myid==Host_ID) printf("Format error for COHP.pairs\n");
	po++;
      }
    }
  } 
  ...
  ****************************************************/
  ...
  if (Wannier_Func_Calc){
    ...
    /* ---start bbprasad--- */
    input_double("CWF.disentangling.Erange.Bottom",&CWF_disentangling_Erange_Bottom,(double)-1000.0); /* eV */
    input_double("CWF.disentangling.Erange.Top",&CWF_disentangling_Erange_Top,(double)0.0); /* eV */
    input_double("CWF.disentangling.smearing.kBT0",&CWF_disentangling_smearing_kBT0,(double)0.3); /* eV */
    input_double("CWF.disentangling.smearing.kBT1",&CWF_disentangling_smearing_kBT1,(double)0.3); /* eV */
    input_double("CWF.disentangling.smearing.bound",&CWF_disentangling_smearing_bound,(double)1.0e-12); /* a small constant */  
  ...
  ***************************************************************/
  ...
  /****************************************************
    Hybrid functionals
    by the efficient exchange operator method
  ****************************************************/

  if (XC_switch==5){
    
    input_double("scf.xmin.Rec.Coulomb",&xmin_Rec_Coulomb,-14.0);
    input_double("scf.xmax.Rec.Coulomb",&xmax_Rec_Coulomb, 14.0);
    input_int("scf.Ng1.Rec.Coulomb",&Ng1_Rec_Coulomb,11); 
    input_int("scf.Nrank.Rec.Coulomb",&Nrank_Rec_Coulomb,900);

    Allocate_Arrays(15);
  }

  /*************************************************************
    polarization calculation by Analytic Berry Connection (ABC)
  *************************************************************/

  input_logical("scf.Polarization.ABC",&Polarization_ABC_flag,0); /* default=off */

  /*************************************************************
    calculation of the analytic Berry connections for indiviual 
    bands and relevant quanties. 
  *************************************************************/

  input_logical("calc.ABC.flag",&Calc_ABC_flag,0); /* default=off */
  ...
  ****************************************************/
  ...
  if (myid==Host_ID && MYID_MPI_COMM_WORLD==Host_ID && 0<level_stdout){  
